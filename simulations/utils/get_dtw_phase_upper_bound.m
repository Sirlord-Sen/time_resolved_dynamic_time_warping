function [dtw_max_dist, dtw_norm_max_dist] = get_dtw_phase_upper_bound(x, win_size, gamma, F, Fs, phi_max, num_trials)
% get_dtw_phase_upper_bound - Calculate the upper bound of DTW distances between a sine wave 
% and a randomly phase-perturbed version using multiple trials.
%
% Syntax:
%   [dtw_max_dist, dtw_norm_max_dist] = get_dtw_phase_upper_bound(x, win_size, gamma, F, Fs, phi_max, num_trials)
%
% Parameters:
...
%   phi_max     - Maximum phase deviation (in radians) for the random phase perturbation.
%   num_trials  - Number of trials to perform for the random phase perturbation (integer).
%
% Returns:
...
%   dtw_norm_max_dist  - Array of normalized DTW distances for each trial (DTW distance divided by path length).
    ...
    % Loop to calculate DTW distances for each random phase perturbation trial
    for i = 1:num_trials
        % Create random phase noise between 0 and phi_max
        random_phase_noise = phi_max * rand(size(t));
        
        % Generate the second sine wave with random phase perturbation
        y = sin(2 * pi * F * t + random_phase_noise);
